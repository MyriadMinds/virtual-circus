...
impl Window {
  pub(crate) fn new(vulkan: &Vulkan, glfw_window: glfw::Window, mut resources: WindowResources) -> Result<Self> {
    ...
    if swapchain_images.len() > resources.depth_images.len() {
      return Err(EngineError::CreationError("Not enough depth buffers to assign one to every swapchain image"));
    }

    resources.depth_images.truncate(swapchain_images.len());
    ...
    let graphics_pipeline = Pipeline::new(&device, &graphics_pipeline_layout, &swapchain.format)?;
  ...
  pub(crate) unsafe fn record_commands(&self, command_buffer: &vk::CommandBuffer, image_index: usize, models: &[impl Model]) -> Result<()> {
    ...
    let depth_attachment = [vk::RenderingAttachmentInfo {
      image_view: *self.depth_image_views[image_index],
    ...
    let mut rendering_context = RenderingContext::new(device, command_buffer, &self.graphics_pipeline_layout, time);

    device.begin_command_buffer(*command_buffer, &begin_info)?;
    self.transition_swapchain_image(command_buffer, &self.swapchain_images[image_index], RenderingStage::BeforeRender);
    ...
    models.iter().for_each(|model| model.draw(&mut rendering_context));
    device.cmd_end_rendering(*command_buffer);
    self.transition_swapchain_image(command_buffer, &self.swapchain_images[image_index], RenderingStage::AfterRender);
    device.end_command_buffer(*command_buffer)?;

    trace!("Finished recording command buffer!");
    Ok(())
  }
  ...
  pub(crate) fn draw_frame(&mut self, models: &[impl Model]) -> Result<()> {
    unsafe {
      ...
      // Wait for fence reset until after acquiring image to prevent deadlocks due to recreating the swapchain
      device.reset_fences(&[**fence])?;
      device.reset_command_buffer(*command_buffer, vk::CommandBufferResetFlags::empty())?;
      self.record_commands(command_buffer, image_index as usize, models)?;
...
fn create_global_descriptor_set_info(swapchain_extent: &vk::Extent2D) -> GlobalDescriptorSetInfo {
  let camera_pos = glam::Vec3::new(1.0, 1.0, 1.5);
  let center_pos = glam::Vec3::new(-1.0, -1.0, -0.5);
  let up_direction = glam::Vec3::new(0.0, 0.0, -1.0);
  let view = glam::Mat4::look_to_lh(camera_pos, center_pos, up_direction);
  ...
  let z_near = 0.1;
  let z_far = 10.0;
  let projection = glam::Mat4::perspective_lh(fov_y_radians, aspect_ratio, z_near, z_far);

  GlobalDescriptorSetInfo {
    view,
    projection,
    model: glam::Mat4::from_rotation_z(2.0),
  }
}
